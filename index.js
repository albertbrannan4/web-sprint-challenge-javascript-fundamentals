...
// Using closure, the console.log(internal) is able to reach one layer outside of it's scope to access the internal variable containing the value "Hello! I'm inside myFunction!".
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(array){
    const displayNames = []
    array.forEach((each)=>{
      displayNames.push(`name: ${each.animal_name}, scientific: ${each.scientific_name}`)
    })
    return displayNames;
  }
  ...
  */
  ...
  function lowerCaseNames(array){
    return array.map(each=>{
      return each.animal_name.toLowerCase();
    })
  }
  ...
  */
  ...
  function USApop(array){
    return array.reduce(
      (accumulator, each) => accumulator + each.population,
      0
    );
  }
